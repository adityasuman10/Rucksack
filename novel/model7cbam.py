...
```
def create_coatnet_model(num_classes, image_size, dropout_rate=0.5):  # Increased dropout
    inputs = layers.Input(shape=(image_size, image_size, 3))

    # Data augmentation (only during training)
    augmentation = create_augmentation_layer()

    # Stem (Initial convolution) - Reduced channels
    x = layers.Conv2D(32, 3, strides=1, padding='same')(inputs)  # Reduced from 64 to 32
    x = layers.BatchNormalization()(x)
    x = layers.Activation('gelu')(x)
    x = layers.Conv2D(32, 3, padding='same')(x)  # Reduced from 64 to 32

    # Stage 1: MBConv-like blocks - Keeping both blocks, reducing parameters
    x = mbconv_block_with_multiscale(x, 32, 32, expansion_factor=2)  # Reduced expansion from 4 to 2
    x = mbconv_block_with_multiscale(x, 32, 32, expansion_factor=2)  # Reduced expansion from 4 to 2

    # Stage 2: MBConv-like blocks with stride - Keeping both blocks
    x = mbconv_block_with_multiscale(x, 32, 64, expansion_factor=2, stride=2)  # Reduced expansion from 4 to 2
    x = mbconv_block_with_multiscale(x, 64, 64, expansion_factor=2)  # Reduced expansion from 4 to 2

    # Stage 3: Transformer-like blocks - Keeping both blocks, reducing parameters
    x = transformer_block(x, 64, 128, num_heads=4)  # Reduced from 8 to 4 heads, 256 to 128 channels
    x = transformer_block(x, 128, 128, num_heads=4)  # Reduced from 8 to 4 heads, kept same channels

    # Stage 4: Final transformer blocks - Keeping both blocks, reducing parameters
    x = transformer_block(x, 128, 256, num_heads=4, stride=2)  # Reduced from 512 to 256 channels, 8 to 4 heads
    x = transformer_block(x, 256, 256, num_heads=4)  # Reduced from 512 to 256 channels, 8 to 4 heads
